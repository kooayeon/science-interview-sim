...
아래 파일들을 레포에 그대로 추가하면 동작합니다. **Streamlit Community Cloud** 배포 기준으로 작성했습니다.
...
```
# Streamlit Cloud > Project > Settings > Secrets
OPENAI_API_KEY = "sk-..."
```

※ 키가 없으면 STT/자동피드백만 비활성화되고 나머지 기능은 정상 동작합니다.
...
* **평가 항목/가중치**: `gpt_feedback()` 프롬프트와 슬라이더 라벨/범위를 수정
* **타이머 기본값**: `init_state()`의 `timer_sec` 바꾸고, 사이드바 슬라이더 범위 조정
* **오프라인 STT**: `stt_whisper()` 대신 로컬 Whisper/ffmpeg 래퍼 함수를 만들어 교체 가능
* **질문 뱅크 대량 주입**: `.csv/.txt` 업로드 형식만 맞추면 자동 인식
